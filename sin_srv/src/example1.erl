...
loop() ->
  io:format("###########     [~p:~p] LOOP PID: ~p~n", [?MODULE, ?FUNCTION_NAME, self()]),
  receive
    {job_1, N, ok} ->
...
job_1_loop(N) ->
  receive
    R = {master, From, _Msg} ->
      io:format("###########     [~p:~p(~p)] Received (1): ~p~n", [?MODULE, ?FUNCTION_NAME, N, R]),
      sin_proc:master_pid(From) ! {job_1, N, ok};
    Any ->
      io:format("###########     [~p:~p(~p)] Received (2): ~p~n", [?MODULE, ?FUNCTION_NAME, N, Any])  
  end,
  job_1_loop(N).
